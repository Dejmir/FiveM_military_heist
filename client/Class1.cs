...
namespace fivem_military_heist
{
    public class Class1 : BaseScript
    {
        ...
        private async void ContactStart(string action)
        {
            ...
            float heading = 0;
            if (rng == 1) { pos = new Vector3(RandomSpawnPoint1[0], RandomSpawnPoint1[1], RandomSpawnPoint1[2]); heading = RandomSpawnPoint1[3]; }
            if (rng == 2) { pos = new Vector3(RandomSpawnPoint2[0], RandomSpawnPoint2[1], RandomSpawnPoint2[2]); heading = RandomSpawnPoint2[3]; }
            if (rng == 3) { pos = new Vector3(RandomSpawnPoint3[0], RandomSpawnPoint3[1], RandomSpawnPoint3[2]); heading = RandomSpawnPoint3[3]; }
            if (rng == 4) { pos = new Vector3(RandomSpawnPoint4[0], RandomSpawnPoint4[1], RandomSpawnPoint4[2]); heading = RandomSpawnPoint4[3]; }
        ...
        private void ShowNotification(string text, bool manual = false)
        {
            ...
            if(!manual) Function.Call(Hash.END_TEXT_COMMAND_THEFEED_POST_MESSAGETEXT, "heisthud", "kiaoverlay", false, 1, "Kontakt", "");
        ...
        private async Task WorkingMarkers()
        {
            ...
            if (LocalPlayer.Character.Position.DistanceToSquared(enterBaseVector) < 1.2f && !insideBase)
            {
                ...
                if(API.IsControlJustReleased(1, 51))
                {
                    ...
                    if(heist_type == "silent") TriggerServerEvent("military_heist:removeitem:server", LocalPlayer.ServerId, "fajnyheist_enter_silent", 1);
